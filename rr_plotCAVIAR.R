...
#colnames(a)=c('ps','r2')
b=read.table('ttt',header=F)
h=read.table('cs6.bed',header=F)
colnames(h)=c('ps','sc')
h$ps=h$ps/1e6
...
png(paste(args[1],"/",args[2],"/",args[2],"_tmp.png",sep=""),height=6000,width=6000,res=700)
...
pdf(paste(args[1],"/",args[2],"/",args[2],"_tmp.pdf",sep=""),height=9,width=9)
#plot(dat1$logp~dat1$ps,xlab='position',ylab='-log(p)',cex.lab=1.3,xlim=range(dat$ps),
     #ylim=c(0,ymax*1.2),xaxt='n',col='navy',cex=0.8)
